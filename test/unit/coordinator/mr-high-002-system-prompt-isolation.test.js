...
// the rl-pipeline pipeline so the { system, user } message shape reaches
...
test('coordinate forwards system_prompt through to rl_pipeline pipeline', async () => {
...
test('TASK_GUIDANCE exports a map with all expected task_type keys', () => {
  ...
  // and the mirror in rl-pipeline/src/parallel.js.
...
test('TASK_GUIDANCE wording matches rl-pipeline mirror (cross-repo invariant)', async () => {
  // Import the rl-pipeline-side mirror to verify they stay in sync.
  const rlPipelinePath = new URL('../../../../rl-pipeline/src/parallel.js',
    import.meta.url).pathname;
  let rlPipelineTaskGuidance;
  try {
    // The map is module-local in rl-pipeline; we can't import it directly,
    // so we read it as text and extract the values.
    const { readFileSync } = await import('node:fs');
    const text = readFileSync(rlPipelinePath, 'utf8');
    for (const key of Object.keys(TASK_GUIDANCE)) {
      const re = new RegExp(`\\b${key}:\\s*'([^']+)'`);
      const m = text.match(re);
      assert.ok(m, `rl-pipeline parallel.js must define TASK_GUIDANCE.${key}`);
      assert.equal(m[1], TASK_GUIDANCE[key],
        `TASK_GUIDANCE.${key} diverges between AWARE and rl-pipeline`);
    }
  } catch (e) {
    // If the rl-pipeline path is not available (CI without rl-pipeline),
