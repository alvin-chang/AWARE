...
test('online mode uses online clients and skips offline', async () => {
  const online = alwaysWorks('the provider', { reasoning: 'from the provider' });
  ...
  assert.equal(result.reasoning, 'from the provider');
  assert.equal(result._routed_via, 'the provider');
  assert.equal(r.whichClient(), 'the provider');
});

test('online mode throws if primary fails (no offline fallback)', async () => {
  const online = alwaysFails('the provider', '502 bad gateway');
...
// === Hybrid mode (the ADR (internal) default) ===

test('hybrid mode uses primary on success', async () => {
  const provider = alwaysWorks('the provider', { reasoning: 'from the provider' });
  const ollama = ollamaClient('ollama', { response: { reasoning: 'from ollama' } });
  const r = makeModelRouter({ clients: [provider, ollama], mode: 'hybrid' });
...
test('hybrid mode falls back to Ollama when provider fails', async () => {
  const provider = alwaysFails('the provider', '502 from the provider');
  const ollama = ollamaClient('ollama', { response: { reasoning: 'from ollama' } });
  const r = makeModelRouter({ clients: [provider, ollama], mode: 'hybrid' });
...
test('offline mode uses only offline (Ollama) clients', async () => {
  // Even if we pass a "the provider" client, offline mode should not invoke it
  let providerCalled = false;
  const provider = mockClient('the provider', {
    generate: async () => { providerCalled = true; return { reasoning: 'should not happen' }; },
  });
  const ollama = ollamaClient('ollama', { response: { reasoning: 'from ollama offline' } });
  const r = makeModelRouter({ clients: [provider, ollama], mode: 'offline' });

  const result = await r.generate('hi');
  assert.equal(result.reasoning, 'from ollama offline');
  assert.equal(result._routed_via, 'ollama');
  assert.equal(providerCalled, false, 'the provider should not be called in offline mode');
});
...
test('offline mode throws if no offline client is configured', async () => {
  ...
  const online = alwaysWorks('the provider');
  assert.throws(
...
test('router skips client whose health() returns false (cached)', async () => {
  ...
  const r = makeModelRouter({ clients: [provider, ollama], mode: 'hybrid', healthTtlMs: 60_000 });

  const result = await r.generate('hi');
  assert.equal(result.reasoning, 'from ollama');
  assert.equal(providerCalled, false, 'the provider must be skipped on health probe failure');
});
...
test('health() reports per-client status and overall ok', async () => {
  const provider = mockClient('the provider', { healthy: () => Promise.resolve(true) });
  ...
  assert.equal(h.clients['the provider'].ok, true);
