...
//
// SC-CRITICAL-002 (security audit 2026-06-25): bearer-token auth
...
test('SC-CRITICAL-002: /coordinate with no Authorization header returns 401 when auth enabled', async () => {
...
test('SC-CRITICAL-002: /coordinate with wrong Bearer token returns 401', async () => {
  await withServerAuth({ port: 0, router: stubRouter() }, { AUTH_DISABLED: false, TOKEN: VALID_TOKEN }, async (h) => {
    const res = await fetch(`http://${h.host}:${h.port}/coordinate`, {
      method: 'POST',
      headers: {
...
test('SC-CRITICAL-002: /coordinate with non-Bearer auth scheme returns 401', async () => {
  await withServerAuth({ port: 0, router: stubRouter() }, { AUTH_DISABLED: false, TOKEN: VALID_TOKEN }, async (h) => {
    const res = await fetch(`http://${h.host}:${h.port}/coordinate`, {
      method: 'POST',
      headers: {
...
test('SC-CRITICAL-002: /coordinate with valid Bearer token is NOT short-circuited by auth gate', async () => {
...
test('SC-CRITICAL-002: /budget/status requires auth same as /coordinate', async () => {
  await withServerAuth({ port: 0, router: stubRouter() }, { AUTH_DISABLED: false, TOKEN: VALID_TOKEN }, async (h) => {
...
test('SC-CRITICAL-002: /health is public (liveness probes must work)', async () => {
  await withServerAuth({ port: 0, router: stubRouter() }, { AUTH_DISABLED: false, TOKEN: VALID_TOKEN }, async (h) => {
...
test('SC-CRITICAL-002: /version is public (orchestrators query it)', async () => {
  await withServerAuth({ port: 0, router: stubRouter() }, { AUTH_DISABLED: false, TOKEN: VALID_TOKEN }, async (h) => {
...
test('SC-CRITICAL-002: AWARE_COORDINATOR_AUTH_DISABLED=1 lets all routes through', async () => {
