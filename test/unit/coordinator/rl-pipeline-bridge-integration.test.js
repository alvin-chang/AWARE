// AWARE 2.0 coordinator — rl-pipeline-bridge integration tests
// These tests use mock clients (no network). The live API path is in rl-pipeline's
...
import {
  ...
  classifyError,
  DEFAULT_PAIRS_DIR,
} from '../../../src/coordinator/rl-pipeline-bridge-integration.js';
...
test('awareRlPipeline wraps rl_pipeline and returns ok:true on success', async () => {
...
test('awareRlPipeline maps confidence → autonomy_level across L1/L2/L3 (HO-HIGH-001)', async () => {
  ...
  const r1 = await awareRlPipeline({
...
test('awareRlPipeline classifies invalid_input errors', async () => {
  const result = await awareRlPipeline({
...
test('awareRlPipeline writes preference pairs to a daily JSONL file', async () => {
...
test('awareRlPipeline returns pair_path: null when writePairs is false', async () => {
  const result = await awareRlPipeline({
    problem: 'p',
    K: 1,
    task_type: 'simple',
...
test('DEFAULT_PAIRS_DIR is under redacted-host-config/redacted-internal-project/preference-pairs/', () => {
  assert.match(DEFAULT_PAIRS_DIR, /\.<runtime>\/redacted-internal-project\/preference-pairs$/);
});
...
test('coordinate() is the public entry point and accepts session/agent context', async () => {
  const result = await coordinate({
    ...
    client: mockClient(),
    sessionId: 'sess-123',
    agentId: 'agent:researcher:live-001',
  });
  assert.equal(result.ok, true);
});
