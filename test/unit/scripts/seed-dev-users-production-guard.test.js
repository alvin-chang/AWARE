...
//
// SC-CRITICAL-005 (security audit 2026-06-25): scripts/seed-dev-users.js
...
test('SC-CRITICAL-005: seed-dev-users.js refuses to run with NODE_ENV=production', () => {
...
test('SC-CRITICAL-005: seed-dev-users.js exits with the SC-CRITICAL-005 citation', () => {
  const result = spawnSync(process.execPath, [SCRIPT_PATH], {
    env: { ...process.env, NODE_ENV: 'production' },
    encoding: 'utf8',
    timeout: 5000,
  });
  assert.match(result.stderr, /SC-CRITICAL-005/, 'stderr should cite the SC-CRITICAL-005 finding');
});

test('SC-CRITICAL-005: seed-dev-users.js runs in development mode (NODE_ENV unset)', () => {
