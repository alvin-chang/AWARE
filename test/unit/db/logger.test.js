...
test('logger: logConversationFireAndCodert never throws on missing args', async () => {
  const { logConversationFireAndCodert } = await import('../../../src/db/logger.js');
  // Should not throw synchronously
  assert.doesNotThrow(() => {
    logConversationFireAndCodert(null);
    logConversationFireAndCodert(undefined);
    logConversationFireAndCodert({});
  });
...
test('logger: fire-and-forget logs to stderr when logConversation returns not-logged', async () => {
  ...
  // logConversationFireAndCodert's `.then(r => { if (!r.logged) ... })`
  ...
  try {
    const { logConversationFireAndCodert } = await import('../../../src/db/logger.js');
    logConversationFireAndCodert({}); // missing required fields
...
// NOTE: The `.catch` handler in logConversationFireAndCodert (line 157-160)
...
test('logger: fire-and-forget never rejects (the .catch is defensive dead code)', async () => {
  ...
  // assert logConversationFireAndCodert never produces an unhandled
