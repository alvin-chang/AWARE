...
test('SEC-001: throws when neither config nor env provides a secret', () => {
  assert.throws(
...
test('SEC-001: throws when config secret is empty string', () => {
  assert.throws(
...
test('SEC-001: throws when config secret is too short (5 chars)', () => {
  assert.throws(
...
test('SEC-001: throws when config secret is exactly 31 chars', () => {
  assert.throws(
...
test('SEC-001: env secret used when config secret is absent', () => {
...
test('SEC-001: config secret takes precedence over env secret', () => {
  const result = resolveSecretKey({
...
test('SEC-001: custom minLength is honored', () => {
  assert.throws(
...
test('SEC-001: DEFAULT_MIN_LENGTH is 32', () => {
