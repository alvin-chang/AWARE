...
describe('Layer 1: pre-commit-check.sh content rules', () => {
  ...
  test('BLOCKS LAN IP 192.168.x.x in STATUS.md', () => {
    const tmp = makeFixtureRepo({
      ...
      statusMdContent: 'Gitea is at http://192.0.2.99:4001/example/aware\n',
      usersJsonTracked: false,
      usersTemplateContent: null,
    });
  ...
  test('BLOCKS LAN IP 10.x.x.x in STATUS.md', () => {
    const tmp = makeFixtureRepo({
      docsAuditsContent: null,
      statusMdContent: 'Internal: http://198.51.100.99:8080\n',
      usersJsonTracked: false,
      usersTemplateContent: null,
    });
...
// ─── C309 — seed-dev-users env-var salt + L4 public CI guard ────────────

const SEED_DEV_USERS = path.join(REPO_ROOT, 'scripts', 'seed-dev-users.js');
const L4_PUBLIC_CI = path.join(REPO_ROOT, '.github', 'workflows', 'lint-private-data.yml');

describe('C309: scripts/seed-dev-users.js salt pattern', () => {
  test('script exists', () => {
    expect(fs.existsSync(SEED_DEV_USERS)).toBe(true);
  });

  test('uses SEED_DEV_SALT env var (C309 mandate)', () => {
    const src = fs.readFileSync(SEED_DEV_USERS, 'utf8');
    expect(src).toMatch(/SEED_DEV_SALT/);
  });

  test('does NOT contain a hard-coded hex-salt literal (C309 anti-pattern)', () => {
    const src = fs.readFileSync(SEED_DEV_USERS, 'utf8');
    // The old anti-pattern was: const FIXTURE_SALT = 'fdf2b932...';
    // C309 mandates SEED_DEV_SALT env var with random default.
    expect(src).not.toMatch(/FIXTURE_SALT\s*=\s*['"][a-f0-9]{32,}['"]/);
  });

  test('falls back to a random salt when SEED_DEV_SALT is unset', () => {
    const src = fs.readFileSync(SEED_DEV_USERS, 'utf8');
    // The fallback should use crypto.randomUUID() so each `npm run seed:dev`
    // invocation produces different hashes (removes rainbow-table risk).
    expect(src).toMatch(/crypto\.randomUUID|crypto\.randomBytes/);
  });
});

describe('C309: Layer 4(a) public github CI guard', () => {
  test('.github/workflows/lint-private-data.yml exists', () => {
    expect(fs.existsSync(L4_PUBLIC_CI)).toBe(true);
  });

  test('L4 workflow has the 4 jobs: lint-host-paths, lint-status-md, lint-fixture-data, gitleaks-scan', () => {
    const src = fs.readFileSync(L4_PUBLIC_CI, 'utf8');
    expect(src).toMatch(/lint-host-paths:/);
    expect(src).toMatch(/lint-status-md:/);
    expect(src).toMatch(/lint-fixture-data:/);
    expect(src).toMatch(/gitleaks-scan:/);
  });

  test('L4 lint-fixture-data blocks committed FIXTURE_SALT (C309 anti-pattern)', () => {
    const src = fs.readFileSync(L4_PUBLIC_CI, 'utf8');
    expect(src).toMatch(/FIXTURE_SALT/);
    expect(src).toMatch(/SEED_DEV_SALT/);
  });

  test('L4 workflow triggers on pull_request + push to main', () => {
    const src = fs.readFileSync(L4_PUBLIC_CI, 'utf8');
