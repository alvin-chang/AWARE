/**
 * ADR-009: Phase 2.1 — Pheromone Specialists Tests
 * 
 * Test scenarios per ADR-009 §Test Requirements:
...
console.log('\n=== ADR-009 Pheromone Specialists Tests ===\n');
...
test('T1: CODE_GENERATION task → pheromones deposited in code-generation table', () => {
  ...
  const pathAgents = ['orchestrator', 'agent-code-1'];
  ...
  assertTrue(table.transitions.has('orchestrator'), 'orchestrator transition exists');
  assertTrue(table.transitions.get('orchestrator').has('agent-code-1'), 'orchestrator→agent-code-1 transition exists');
});
...
test('T2: RESEARCH pheromones do NOT appear in CODE_GENERATION table', () => {
  ...
  depositPheromone({
    ...
    pathAgents: ['orchestrator', 'agent-research-1'],
