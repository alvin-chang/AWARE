...
This document describes the centralized configuration system for AWARE 2.0, implemented as of commit `880a47c`. See [ADR-021](adr/ADR-021.md) for the architectural decision and alternatives considered.
...
| `minimaxKey`     | `LLM_API_KEY`      | string | (none)                                           | **Secret** — redacted in snapshot as `<unset>` or `<redacted length=N>` |
...
| Property | Env var                  | Type   | Default                                                       | Notes                          |
|----------|--------------------------|--------|---------------------------------------------------------------|--------------------------------|
| `path`   | `AWARE_HEAVY_THINK_PATH` | string | `<repo>/../../heavy-think/src/index.js` (sibling-repo path)   | HeavySkill reasoning layer     |

### `config.warnings`

Array of soft validation issues, accumulated by `config.warnings()`. Examples:

- `LLM_API_KEY not set; required in online mode`
...
config.model.minimaxKey       // reads LLM_API_KEY (secret; redact in snapshot)
...
- `mode=online` but `LLM_API_KEY` is unset (no API key to call → hard fail)
...
```javascript
{
  ...
  model:       { mode: 'hybrid', minimaxKey: '<unset>' or '<redacted length=125>', ollamaUrl: 'http://127.0.0.1:11434' /* minimaxHost appears only when set */ },
  heavyThink:  { path: '/Users/alfie/src/heavy-think/src/index.js' },
  warnings:    []
}
```

> **JSON.stringify note:** `minimaxHost` is omitted from the JSON output when undefined (which is the common case). It appears in the JS object literal as `minimaxHost: undefined`, but `JSON.stringify` drops undefined values.

**Never write the value of `config.model.minimaxKey` (or any other secret) to logs/telemetry. Use `config.snapshot()`.**
...
```bash
cd /Users/alfie/src/AWARE && node -e "const c = require('./src/config/index.cjs'); console.log(JSON.stringify(c.snapshot(), null, 2))"
```

### Run the config tests

```bash
cd /Users/alfie/src/AWARE && node --test test/unit/config/index.test.js
```

## See also

- [ADR-021](adr/ADR-021.md) — the architectural decision
- [ADR-020](adr/ADR-020.md) — AWARE 2.0 architecture
