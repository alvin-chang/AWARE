...
start with [docs/adr/ADR-020.md](docs/adr/ADR-020.md) (the v2 architecture
decision) and the per-phase SOPs in [docs/sop/](docs/sop/).
...
| File | Exports |
|------|---------|
| `src/coordinator/index.js` | `coordinate`, `buildDefaultRouter`, `makeModelRouter`, `makeOllamaHealth`, `awareHeavyThink` |
| `src/coordinator/http-server.js` | `startServer` |
| `src/coordinator/model-router.js` | `makeModelRouter`, `makeOllamaHealth` |
| `src/coordinator/heavyskill-integration.js` | `awareHeavyThink`, `buildPairPath`, `classifyError`, `K_CONFIGS` |
| `src/gateway/server.js` | `app`, `GATEWAY_VERSION`, `isKilled` (CJS) |
| `src/db/index.js` | `getPool`, `runMigrations`, `closePool` |
| `src/db/logger.js` | `logConversation`, `logConversationFireAndForget`, `_resetForTest` |
...
- `docs:` — documentation only (README, <internal-doc>, docs/, sop/)
...
update the AWARE call sites in `src/coordinator/heavyskill-integration.js`.
...
1. **Read** `docs/adr/ADR-020.md` — the v2 master architecture decision
2. **Read** the relevant per-phase SOPs in `docs/sop/` — they often call
...
The repo's `<internal-doc>` records every phase closure with the commit hash.
The `<host-config>/sops/` mirror is the canonical SOP store for the
operator's runtime.
...
When the PR is merged, mirror any new SOPs to `<host-config>/sops/`.
...
- [<internal-doc>](<internal-doc>) — closure status of every phase, with commit hashes
- [docs/adr/ADR-020.md](docs/adr/ADR-020.md) — the v2 master architecture decision
- [docs/config.md](docs/config.md) — full config reference (auto-generated from `src/config/index.cjs`)
- [docs/sop/](docs/sop/) — Standard Operating Procedures per phase
- [docs/adr/](docs/adr/) — full ADR index (ADR-009 through ADR-021)
