...
# -- Modal App wrapper (R2) -----------------------------------------------
#
# This block is executed ONLY when the file is imported by the Modal
# CLI (`modal deploy training/run.py`). Plain `python3 -m training.run`
# exits at the `if __name__ == "__main__"` line above and never reaches
# here, so the modal SDK is not imported on the CLI path.
#
# The decorator registers a function named "train" in Modal App
# "aware-trainer". The Node poller (src/trainer/modal-client.js) looks
# up the function via `client.functions.fromName("aware-trainer", "train")`
# and calls `fn.spawn([runId, datasetPath, config], {})`.
#
# Operator action (one-time, no GPU cost — just image registry push):
#     MODAL_PROFILE=goodciso modal deploy training/run.py
# (The profile name is operator-side CLI shorthand; see Phase 3 SOP.)

try:
    import modal  # noqa: E402  (deferred — modal is not a CLI dep)

    # Read image/GPU/volume/timeout from the same config the Node
    # poller uses, so the two never drift. If the config is missing
    # (e.g. someone deployed from a stripped-down checkout), fall
    # back to the documented defaults from ADR-020.
    ...
    _app = modal.App("aware-trainer")

    # Container image. We build from the same Dockerfile the poller's
    # config references, so the deployed function and the manual-debug
    # `docker run` path share the exact same image.
    _image = modal.Image.from_dockerfile(
        _modal_config.get("image_dockerfile", "Dockerfile.training")
    ).env({
        # Bake the base-model default into the image so a stale deploy
        # of an old run.py doesn't regress to the deprecated training-optimizer
        # pre-quantized Qwen 2.5 build. Override per-job via
        # AWARE_BASE_MODEL env var in the function kwargs.
        "AWARE_BASE_MODEL": _modal_config.get("dpo_defaults", {}).get(
            "base_model", "Qwen/trained-model"
        ),
        ...
        "AWARE_DPO_BATCH_SIZE": str(_modal_config.get("dpo_defaults", {}).get(
            "per_device_train_batch_size", 4
        )),
        "AWARE_DPO_GRAD_ACCUM": str(_modal_config.get("dpo_defaults", {}).get(
    ...
    # GPU spec — single A100-80GB per ADR-020 Decision 2.
    ...
    def train(run_id: str, dataset_path: str, config: dict) -> int:
        ...
        Contract (matched by src/trainer/modal-client.js:264-267):
            fn.spawn([runId, datasetPath, config], {})

        We write `config` to <volume>/config.json so the existing main()
        code path can read it via load_config() — keeps the CLI and
        Modal paths unified (one config loader, not two).
        """
        # Mount the volume into the container's filesystem (Modal
        # requires an explicit reload after writes).
        ...
        # Synthesize the same argv that the trainer service would
        # have built for a direct `python3 -m training.run` invocation.
        sys.argv = [
            "training.run",
            "--config", config_path,
    ...
    # modal not installed — CLI path still works, deploy path will
    # raise a clear ImportError when `modal deploy` runs.
    pass
except Exception as _e:  # noqa: BLE001
    # Any other failure during App construction (e.g. bad config
    # JSON) should not break the CLI path. Log and continue.
    log("modal_app_init_failed", error=str(_e)[:500])
