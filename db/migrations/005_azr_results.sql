...
-- new `azr_result` rule, which gates MetaClaw preference pairs against
-- AZR pass/fail signals (ADR-020 Decision 2: "AZR outcome filter
-- gates MetaClaw process training — signals are NOT merged at the
-- reward level").
--
-- This table is the *join surface* between the two training corpora:
-- - MetaClaw pairs come from heavy-think preference-pair JSONL files,
...
CREATE TABLE IF NOT EXISTS aware_azr_results (
  ...
  -- The actual join surface — the MetaClaw side needs this to match
  -- against. Heavy-think writes the same hash into each preference
...
-- Join index: how the outcome-filter looks up "does this MetaClaw
