...
//     → throws Error with the SC-CRITICAL-001 message if missing or short
//
...
function resolveSecretKey(opts) {
  ...
  if (!supplied || typeof supplied !== 'string') {
    throw new Error(
      ...
      'SC-CRITICAL-001 (security audit 2026-06-25) — no default value allowed.'
    );
  }
  if (supplied.length < minLength) {
    throw new Error(
      'AWAREEngine: SECRET_KEY must be at least ' + minLength + ' characters (got ' +
      supplied.length + '). SC-CRITICAL-001 (security audit 2026-06-25).'
    );
  }
  return supplied;
}
