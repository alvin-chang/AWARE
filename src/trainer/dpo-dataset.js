// src/trainer/dpo-dataset.js — Phase 4 (ADR-020 618-627, ADR-029 Repair 4)
//
// DPO dataset assembly. Converts filtered preference-pair records into
// DPO training rows in "messages" format, ready for Modal.
//
// This module was extracted from heavy-think's expected `toDpoDataset`
// surface (which was documented in <internal-doc> and referenced in
// trainer/index.js:593, but never actually exported from heavy-think
// — see ADR-028 §Finding 4). Implementing it in AWARE keeps heavy-think
...
export function toDpoDataset(records, options = {}) {
  ...
  for (const rec of records) {
    ...
    // hash set by heavy-think's preference-pair writer.
