...
// Per ADR (internal): plugins own their config surface. The coordinator
// accepts a per-call `pluginConfig` object (the parsed
// `plugins.entries.<id>.config` from the <runtime> shim, e.g. the
// rl-pipeline-bridge v2 shim's `api.pluginConfig`). The coordinator uses it
...
//     the body and forwards; the rl-pipeline integration only
...
//   - Read OC config from disk. The OC shim (in `redacted-internal-source/`)
...
//   - Touch core zod schemas. ADR (internal)'s whole point is that the
//     core schema stays clean; plugin-owned config is opaque to
//     the core.

import config from '../config/index.cjs';

// Lazy-load `defaultKForTaskType` from rl-pipeline's index module
// (which re-exports from `./config.js`). The previous static import
// `'../../../rl-pipeline/src/config.js'` resolved to `/rl-pipeline/config.js`
// in the Docker image (where `WORKDIR=/app`), but the actual location baked
// by Dockerfile.coordinator is `/app/rl-pipeline/config.js`. The static
// path ignored `AWARE_RL_PIPELINE_PATH` and caused the coordinator to
// crash-loop on import-time `ERR_MODULE_NOT_FOUND`. Resolving via
// `config.rlPipeline.path` honors the env override.
//
...
// Tests that need the binding before a call must `await loadRlPipeline()`
...
export function loadRlPipeline() {
  if (_rlPipelineMod) return Promise.resolve(_rlPipelineMod);
  if (_rlPipelineLoadError) return Promise.reject(_rlPipelineLoadError);
  ...
  _rlPipelineLoadPromise = import(config.rlPipeline.path)
    .then((mod) => {
      ...
      if (typeof mod.defaultKForTaskType !== 'function') {
        throw new Error(
          `defaultKForTaskType not exported from ${config.rlPipeline.path} — ` +
...
/**
 * Synchronous wrapper around rl-pipeline's `defaultKForTaskType`.
 * If rl-pipeline has been loaded (via `loadRlPipeline()` or a prior call
 ...
 * canonical rl-pipeline resolution for an unknown task_type should
 * `await loadRlPipeline()` first.
 */
...
export function defaultKForTaskType(task_type) {
  if (_rlPipelineMod && typeof _rlPipelineMod.defaultKForTaskType === 'function') {
    return _rlPipelineMod.defaultKForTaskType(task_type);
  }
  ...
  // matches rl-pipeline v0.2.x K_CONFIGS defaults — drift here is a bug.
  loadRlPipeline().catch(() => { /* surfaced on first await loadRlPipeline() */ });
...
/**
 * Default per-task-type K from rl-pipeline. Resolved lazily on first call
 * via `loadRlPipeline()`. See the module header for why we don't statically
 * import from rl-pipeline/src/config.js.
 *
 * Callers needing the canonical rl-pipeline resolution for an unknown
 * task_type should `await loadRlPipeline()` first; the sync wrapper
 * below returns the static fallback for the common task types.
 */

/**
 * Sanitize a pluginConfig object to the ADR (internal) shape. Returns a new
 ...
 *   4. `defaultKForTaskType(taskType)` — rl-pipeline's per-task-type
...
export function resolveKFromPluginConfig({ explicitK, pluginConfig, taskType }) {
  ...
  //    override (e.g., a model-id prefix `rl-pipeline-bridge-4:` resolved by
  ...
  // 2. Per-agent defaults (S2 in ADR (internal)). Only active when the
  ...
  // 4. Per-task-type fallback (rl-pipeline's own table).
