...
#     1. Writes a HeavySkill-schema JSONL into the trainer container's
...
# bug ledger (bugs #6 + #8 in <internal-doc>):
#   - #6: the HeavySkill schema mismatch (problem/reasoning not
#     prompt/text). The synthetic records here use the correct
#     schema, verified against heavy-think/src/dpo-format.js:28-36.
...
# ─── Synthesize one HeavySkill record ──────────────────────────────────
# The schema (verified against heavy-think/src/dpo-format.js:28-36) is:
...
# matching heavy-think/src/preference-pair.js:hashContent.
synth_record() {
  local i="$1"
  local epoch_ms="$2"
  local problem
  ...
  # Content hash matches heavy-think's hashContent():
  ...
  # Emit one HeavySkill-schema JSONL line.
...
print(f'validated $COUNT HeavySkill-schema records')
"; then
  fail "JSONL validation failed; aborting before writing to the trainer container"
fi
ok "JSONL validated: $LINE_COUNT HeavySkill-schema records"
