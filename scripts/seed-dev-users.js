 ...
 * with a salt that defaults to a random UUID on first run. They are NOT
 * secrets — they're reproducible fixtures: setting SEED_DEV_SALT gives you
 * a known hash for reproducible dev environments; leaving it unset (the
 * default) generates a fresh random salt per `npm run seed:dev` run.
 *
 * The "random salt by default" choice is intentional (C309 / SEC-005):
 * a fixed committed salt + predictable passwords is a known anti-pattern
 * even for dev fixtures — anyone with repo access could rainbow-table it.
 * Random per-run salt preserves the install-time seeder pattern from A1
 * without baking a known salt into the public repo.
 * 
 ...
 * See: RESULT-[date-redacted]-private-data-scrub-private-data-scrub.md for the C309 scrub.
 */
...
// C309: salt now defaults to a random UUID per-run instead of a fixed
// committed value. Set SEED_DEV_SALT explicitly to get reproducible hashes
// across runs (useful for CI snapshot tests); leave unset for fresh salt
// every `npm run seed:dev` invocation.
const FIXTURE_SALT = process.env.SEED_DEV_SALT
  || crypto.randomUUID().replace(/-/g, '');

// SEC-005: refuse to run in
...
if (process.env.NODE_ENV === 'production') {
  ...
  console.error('FATAL: seed-dev-users.js cannot run in NODE_ENV=production. (SEC-005)');
  console.error('The seed users have predictable passwords (admin/user) committed in source.');
  console.error('Use a real provisioning flow in production (see docs/adr/ADR (internal)).');
  process.exit(1);
}
