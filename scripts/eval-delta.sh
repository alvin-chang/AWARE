...
# PASS/FAIL verdict against the ADR (internal) Phase 4 D5 acceptance criterion
...
#   AWARE_EVAL_TARGET_PP   default: 0.03  (the +3pp threshold from ADR (internal))
#
# Exit codes:
#   0 = PASS (delta ≥ target)
...
node -e '
  ...
  console.log("");
  console.log("  ADR (internal) Phase 4 D5 target: aggregate mean ≥ baseline.mean + " + (target*100).toFixed(1) + "pp");
