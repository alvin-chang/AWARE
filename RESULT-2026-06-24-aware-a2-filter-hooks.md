...
**PR URL:** http://<internal-git-host>:4001/alvin/aware/pulls/new/chore/aware-a2-filter-hooks (the release agent to open)
**Status:** ✅ Shipped. Layer 3 install pending the release agent.
...
| `scripts/hooks/pre-receive` | Layer 3 server-side hook (gitea) | ✅ Shipped (the release agent installs) |
...
| `docs/security/gitea-pre-receive-install.md` | the release agent's install guide for Layer 3 | ✅ Shipped |
...
## Environment findings (FYI for the release agent)

- **Local gitea** at `http://<internal-git-host>:4001` runs in a docker container.
  Bare repo path inside the container: `/data/git/repositories/alvin/aware.git/`.
  (Per the install doc — the release agent uses `docker exec gitea ...` to install.)
- **Gitea Actions NOT enabled** — `[actions]` section is absent from
  `/data/gitea/conf/app.ini`. The gitea workflow file is dormant until
  the release agent flips the switch (out of scope for A2).
- **No pre-existing `custom_hooks/`** in the bare repo. the release agent creates it
...
- [ ] Layer 3 install (the release agent's lane) — install doc is ready

## What's pending

- **Layer 3 install on local gitea** — the release agent owns this. Install guide:
  `docs/security/gitea-pre-receive-install.md`
- **Enable gitea Actions** (out of scope for A2) — when the release agent does this,
...
- the auditor audit: `<canonical-credential-store>/workspace-auditor/reports/audit-goodciso-aware-private-data-2026-06-23.md`
